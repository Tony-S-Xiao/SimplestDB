...
std::pair<std::byte*, std::byte*> sdb::SlottedPage::getBlock(OnPagePointer slot_id) {
		...
		OnPagePointer start = (*footer)[2*slot_id];
		OnPagePointer end = start + (*footer)[2*slot_id + 1];
		return { static_cast<std::byte*>(&data_[start]), static_cast<std::byte*>(&data_[end]) };
}
sdb::size16_t sdb::SlottedPage::physicalSize() {
		size16_t space = footer->getFreeSpacePtr();
		size16_t footer_size = footer->physicalSize();
...
sdb::size16_t sdb::SlottedPage::size() {
  // Uses two numbers in the footer for each block
